...
```c++
void fixHeap(vector<int>& arr, int heapSize, int root, int k)
{
	...
	else
	{
		...
		if (left == heapSize)
			...
			largeSubHeap = right;
		if (k >= arr[largeSubHeap])
			arr[root] = k;
		else
		{
			arr[root] = arr[largeSubHeap];
			fixHeap(arr, heapSize, largeSubHeap, k);
		}
	}
}
void constructHeap(vector<int>& arr, int root)
{
	...
	if (left < arr.size())
	{
		...
		fixHeap(arr, arr.size()-1,root,arr[root]);
	}
}
